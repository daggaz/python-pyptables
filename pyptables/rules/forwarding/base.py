...
class ForwardingRule(AbstractRule):
    ...
    def __init__(self, policy, sources, destinations, channels=(), log=False,
                 log_id=None, log_cls=None, comment=None, args=None):
    ...
    def _base_rules(self):
        ...
        if self.policy.upper() == Rule.ACCEPT:
            ...
            raise ValueError('policy must be either %s, %s, %s or %s' % (
                Rule.ACCEPT,
                Rule.DROP,
                Rule.REJECT,
                Rule.NONE,
        ...
        for rule in rules:
            if not self.sources:
                for destination in self.destinations:
                    result.append(rule(
                        args=(destination.as_output(),),
                        comment="%s: route any -> %s" % (
                            rule.comment,
                            destination,
                        ),
                    ))
                ...
                for source in self.sources:
                    for destination in self.destinations:
                        result.append(rule(
                            args=(source.as_input(), destination.as_output()),
                            comment="%s: route %s -> %s" % (
                               rule.comment,
                               source,
                               destination,
